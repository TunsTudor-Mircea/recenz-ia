#!/usr/bin/env python
"""
SVM training script for sentiment classification (full TF-IDF features).

This script trains a Support Vector Machine classifier using all TF-IDF features
without IGWO selection - the classic SVM baseline for text classification.
"""
...
logger = setup_logger(
    name='train_svm_full',
    log_dir=Path('logs'),
    log_filename=f'train_svm_full_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log',
    level=logging.INFO
)


def main():
    """Main training function for SVM with full TF-IDF features."""
    ...
    logger.info("="*80)
    logger.info("SVM SENTIMENT ANALYSIS TRAINING (Full TF-IDF - No IGWO)")
    logger.info("="*80)
    ...
    # Extract TF-IDF features directly (no IGWO)
    logger.info("Extracting full TF-IDF features (no feature selection)...")
    lf_micf_config = feature_config.get('lf_micf', {})
    
    vectorizer = TfidfVectorizer(
        min_df=lf_micf_config.get('min_df', 3),
    ...
    X_test = vectorizer.transform(test_preprocessed)

    logger.info(f"Using all {X_train.shape[1]} TF-IDF features (no IGWO selection)")

    # Save vectorizer
    joblib.dump(vectorizer, experiment_dir / 'tfidf_vectorizer.joblib')
    ...
    if args.grid_search:
        ...
        svm = LinearSVC(C=best_C, max_iter=1000, random_state=42, dual=False)
        svm.fit(X_train_split, y_train_split)
        ...
        # Train with specified C
        logger.info(f"Training LinearSVC with C={args.C}...")
        svm = LinearSVC(C=args.C, max_iter=1000, random_state=42, dual=False)
        svm.fit(X_train_split, y_train_split)
        best_C = args.C
    ...
    with open(experiment_dir / 'hyperparameters.json', 'w') as f:
        json.dump({'C': args.C, 'n_features': X_train.shape[1]}, f, indent=2)
